...
- **Light Cloudflare Worker** (`worker/light-index.ts` → `worker/light/lightApp.ts`) serves the frontend SPA (ASSETS binding) and a minimal API surface: auth (email/password, GitHub OAuth) via D1, GitHub export, and health/status endpoints. It intentionally avoids heavy dependencies (CodeGen DO, ThinkAgent, SpaceDO, containers, sandbox) to stay under the free-tier 3 MiB script size limit.
- Deployment config lives in `wrangler.v2.jsonc` (Worker `vibesdk-v2`, entry `worker/light-index.ts`).

The legacy ThinkAgent/SpaceDO/Artifacts architecture described below has been removed.

## Legacy coding loop (historical, removed)
...
| Shared protocol/type modules | `worker/types/` (agent-schemas, sandbox-types, websocketTypes via `worker/api/websocketTypes.ts`) |
| Go control plane (LLM, codegen, deploys) | `backend/pkg/` |
...
**Preview (dual-plane, option a):** with no `previewUrl`, the preview is
built entirely client-side from `vfsFiles` (`PreviewPanel` →
`buildStaticSrcDoc` → sandboxed `<iframe srcdoc>`); the Go server carries
zero preview load. On chat reopen the VFS snapshot is hydrated once from
`GET /api/projects/:id/files` (read-only, never spawns a room); live WS
updates always win. When `previewUrl` is present (deployed app), the view
switches to the URL-based `PreviewIframe`.
